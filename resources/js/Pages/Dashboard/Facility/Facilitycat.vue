...
const submit = () => {
  form.post(route("facility-category.store"), {
...
const submitEdit = (ic) => {
  editForm.put(route("facility-category.update", ic), {
    onSuccess: () => {
      editForm.reset();
      cancelIsEdit();
    },
...
const deleteInfocat = (ic) => {
  if (confirm("Apakah kamu yakin?")) {
    router.delete(route("facility-category.destroy", ic));
  }
};

const search = () => {
  searchForm.get(
    route("facility-category.index", { search: searchForm.search })
  );
};
</script>
...
<template>
  <section class="container py-4">
    <h1 class="h1">Facility Category</h1>
    <hr class="my-2" />
    ...
    <div v-if="facilitycats.data.length">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        <div
          v-for="(ic, i) in facilitycats.data"
          :key="i"
          class="shadow-md p-3"
        >
